...
--
-- Host: 127.0.0.1    Database: http_server
-- ------------------------------------------------------
...
CREATE TABLE `login_cookies` (
  ...
  `has_logged_out` tinyint(1) NOT NULL DEFAULT '0',
  `logout_time` datetime DEFAULT NULL,
  `last_update_datetime` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `expires_interval_minutes` int NOT NULL DEFAULT '30',
  `remember_me` tinyint NOT NULL DEFAULT '0',
  `ip_address` varchar(255) DEFAULT NULL,
  `user_agent` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`cookie_value`),
  KEY `login_cookies_users_id_fk` (`user_id`),
  CONSTRAINT `login_cookies_users_id_fk` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`)
...
-- Dump completed on 2023-09-10 18:41:19
